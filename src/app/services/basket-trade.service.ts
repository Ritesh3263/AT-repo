...
export class BasketTradeService {
  ...
  async getActivePositionsByBasketId(basket_id:any){
    try{
        let res = await fetch(`${environment.apiBaseUrl}/authenticated-api/brokerage/positions/${basket_id}`, this.getHeaders());
        let data = await res.json();
        return data;
      }
      catch(e: any) {
