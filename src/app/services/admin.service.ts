...
// ------------- Authenticated API Calls to /authenticated-api/user/* ( Login must not be an authenticated request ) ------------- //
  ...
  async getNotifications(pageNumber: number, pageSize: number, sortColumn: string | null = null, sortMode: string | null = null, search: string | null = null) {
    try{
      let res = await fetch(`${environment.apiBaseUrl}/admin-api/notifications?pageNumber=${pageNumber}&pageSize=${pageSize}${sortColumn ? `&sortColumn=${sortColumn}` : ''}${sortMode ? `&sortMode=${sortMode}` : ''}${search ? `&search=${encodeURIComponent(search)}` : ''}`, this.getHeaders());
      let data = await res.json();
      return data;
    }
    catch(e: any) {
  ...
  async getUsers(pageNumber: number, pageSize: number, sortColumn: string | null = null, sortMode: string | null = null, search: string | null = null) {
    try{
      let res = await fetch(`${environment.apiBaseUrl}/admin-api/users?pageNumber=${pageNumber}&pageSize=${pageSize}${sortColumn ? `&sortColumn=${sortColumn}` : ''}${sortMode ? `&sortMode=${sortMode}` : ''}${search ? `&search=${encodeURIComponent(search)}` : ''}`, this.getHeaders());
      let data = await res.json();
      return data;
    }
    catch(e: any) {
